...
public class DeepNeuralNetworkProcessor {
    ...
    public DeepNeuralNetworkProcessor() {
        ...
        // this.net = Dnn.readNetFromTensorflow(model, proto);
        this.net = Dnn.readNet(model, proto);
    }
    ...
    public List<DnnObject> getObjectsInFrame(Mat frame, boolean isGrayFrame) {
        ...
        try {
            ...
            for (int i = 0; i < detections.rows(); ++i) {
                ...
                DnnObject dnnObject = new DnnObject(classId, classNames[classId].toString(), leftPosition,
                        rightPosition, centerPoint);
